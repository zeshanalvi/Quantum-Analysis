...
# -*- coding: utf-8 -*-
"""
Classical SVM on Iris — Master-level Benchmarks (Version-proof hinge loss)
...
Metrics:
  runtime_s, peak_mem_mb, accuracy, hinge_loss, success (acc ≥ acc_tol)
Carbon:
  computed ONLY from Performance runtime (never from scalability/reliability)

CLI kept consistent with your prior scripts.
"""
...
def safe_hinge_loss(y_true, df_scores, classes):
    ...
    # Try newer sklearn API (labels=classes)
    try:
...
def main():
    ...
    # Keep CLI consistent with your other projects
