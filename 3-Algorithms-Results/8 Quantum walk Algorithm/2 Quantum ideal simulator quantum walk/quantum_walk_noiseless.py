...
Master-level empirical benchmark with four metrics:
  1) Performance     – runtime, peak memory, accuracy (TV distance vs dense-unitary reference)
...
def main():
    ...
    if args.workers > 1 and len(jobs) > 1:
        # Prebuild U in parent (cheap) then share via closure (not process-safe), so rebuild per worker:
