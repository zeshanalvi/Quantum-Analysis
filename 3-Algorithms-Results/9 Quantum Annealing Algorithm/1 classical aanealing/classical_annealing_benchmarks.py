...
# -*- coding: utf-8 -*-
"""
Classical Simulated Annealing (Ising/QUBO) — Master-level Benchmarks
...
Benchmarks produced:
  1) Performance     – runtime, peak memory, optimality gap
  2) Scalability     – runtime (log–log) vs N, memory vs N
  3) Reliability     – success rate (gap ≤ tol), gap dispersion
  4) Carbon Footprint– CO₂ from PERFORMANCE runtime only (using your Excel)
...
def make_planted_ising(N: int, p: float, field_strength: float, seed: int):
    """
    Generate ER(N,p) graph; draw planted s★; set J_ij=s★_i s★_j on edges; h_i=field_strength*s★_i.
    Returns: edges(list of (i,j)), J(dict (i,j)->±1), h(np.ndarray), s_star(np.ndarray), E_star(float)
    """
...
def delta_energy_if_flip(i: int, spins: np.ndarray, nbrs_by_i, J, h) -> float:
    """
    ΔE = 2 * s_i * ( sum_{j∈nbrs(i)} J_ij s_j + h_i )
    nbrs_by_i: dict i -> list of (j, J_ij with proper (min,max) index)
    """
    s_i = spins[i]
    field_term = h[i]
    neigh_sum = 0.0
...
def simulated_annealing(N: int, edges, J, h, steps: int, T0: float, Tf: float, seed: int):
    """
    Standard Metropolis SA with linear schedule T_k = T0 + (Tf - T0)*k/(steps-1)
    'steps' = number of sweeps; each sweep visits N spins in random order.
...
def main():
    ...
    # same CLI style as your other projects
