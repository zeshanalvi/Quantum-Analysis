...
# ----------------------------------------------------------------------------------------------------------------
# adiabatic_quantum_noiseless.py
# Ideal (noiseless) quantum statevector simulator for Adiabatic Evolution.
# Benchmarks: Performance, Scalability, Reliability, Carbon (Carbon uses ONLY Performance runtimes).
...
# ------------********------------ Imports
#These imports set up core utilities for command-line control, timing, memory tracking, and parallel execution.
#NumPy and pandas provide numerical processing and tabular data handling for experiments.
#Matplotlib is used for generating plots to visualize benchmark results.
#Warnings are suppressed to keep console output clean during large experimental runs.
...
# ------------********------------
# We approximate exp(-i dt H(s)) with a first-order product over local terms
# at each slice s: apply ZZ, Z, and X rotations noiselessly on the statevector.
...
def Uzz(theta: float) -> np.ndarray:
    # exp(-i * theta * (Z ⊗ Z)) = diag(e^{-i theta}, e^{i theta}, e^{i theta}, e^{-i theta})
...
def simulate_adiabatic_gate_noiseless(n: int, J: float, h: float, Gamma: float,
    ...
    for k in range(m_steps):
        ...
        # H0 term: -Gamma * X  ->  exp(-i dt (1-s) * (-Gamma X)) = exp(i dt (1-s) Gamma X)
        theta_x = (dt * (1.0 - s) * Gamma)
        # H1 terms:
        #  -J * ZZ -> exp(i dt s J ZZ)
        theta_zz = (dt * s * J)
        #  -h * Z  -> exp(i dt s h Z)
        theta_z = (dt * s * h)

        # Build 1q/2q gates for this slice
        U1x = Ux(+theta_x)     # sign handled above
        U1z = Uz(+theta_z)
        U2zz = Uzz(+theta_zz)

        # Apply in a fixed order (first-order product formula within the slice)
        # 1) ZZ on even bonds, then odd bonds to reduce collisions
...
# Dense high-res reference for Reliability (same as classical reference)
...
def plot_runtime_vs_steps(perf_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Runtime vs time slices (Ideal quantum)")
    ax.legend()
...
def plot_memory_vs_steps(perf_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Memory vs time slices (Ideal quantum)")
    ax.legend()
    save_fig(fig, out_path)
...
def plot_error_vs_steps(rel_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Reliability: Error vs time slices (Ideal quantum)")
    ax.legend()
    save_fig(fig, out_path)
...
def plot_error_heatmap(raw_err: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("log10(Error): n vs m (Ideal quantum)")
...
def _one_task(args):
    ...
    tracemalloc.stop()
    perf_row = {"n_qubits": n, "dimension": dim, "steps": m, "repeat": r,
                "runtime_sec": runtime, "peak_mem_mb": peak / (1024 ** 2)}
    return perf_row, psi_m
...
def run_benchmarks(n_list: List[int],
    ...
    tasks = [(n, m, r, J, h, Gamma, T) for n in n_list for m in m_list for r in range(repeats)]
    perf_rows, states = [], []
...
def main():
    parser = argparse.ArgumentParser(description="Ideal (noiseless) quantum adiabatic simulator with four benchmarks and carbon outputs.")
    # Grid / physics
    ...
    print("[INFO] Running ideal quantum adiabatic benchmarks ...")
...
# ------------********------------ CARBON (Performance-only)
    ...
    print(f"[DONE] Ideal quantum adiabatic benchmarks completed in {time.perf_counter() - t_all0:.2f}s")
    print(f"[OUT] Folders under: {root}")
