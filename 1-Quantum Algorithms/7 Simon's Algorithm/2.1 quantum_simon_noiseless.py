...
#This function def gf2_rank () computes the rank of a binary matrix over the finite field GF(2).
#def gf2_rank performs Gaussian elimination using XOR operations to eliminate dependent rows.
#It returns the number of independent rows found during the elimination process.
...
# def gf2_solve_homogeneous_one () solves a homogeneous linear system over GF(2) and 
#   returns one non-zero solution if it exists.
#It reduces the matrix to row-reduced echelon form while tracking pivot and free variables.
#It constructs a solution by assigning a free variable and back-substituting pivot values.
...
# sample_y_orthogonal_to_s () function samples a binary vector y 
#   that is orthogonal to a given vector s over GF(2).
#It repeatedly draws random candidates until the dot product condition y·s = 0 is satisfied.
#It returns a uniformly valid vector from the constrained binary space.
...
# def simon_quantum_noiseless function simulates an ideal noiseless quantum Simon algorithm.
# def simon_quantum_noiseless collects measurement vectors orthogonal to the secret string until sufficient rank is achieved.
...
#def run_single () function runs a single noiseless quantum Simon experiment with a randomly generated hidden string.
#It measures execution time and peak memory usage while attempting to recover the secret.
#It returns a dictionary of performance metrics and success status.
...
#This function resolves the path to an Excel file
#It prefers an existing absolute path, otherwise checks for the file on the Desktop.
#It falls back to returning the original relative path if no match is found.
...
def main():
    ...
    #This block resolves the carbon intensity Excel file path and 
    #    computes carbon emissions from experiment results.
    #It writes per-country and summary carbon data to an Excel file 
    #    and generates basic visualization plots.
    #It handles missing or invalid Excel inputs gracefully by reporting errors and skipping carbon analysis.
...
#This section prints a final summary of generated result files and output directories.
#It reports the locations of performance, scalability, reliability, and carbon analysis outputs.
...
#It ensures the main function is executed only when the script is run directly.
