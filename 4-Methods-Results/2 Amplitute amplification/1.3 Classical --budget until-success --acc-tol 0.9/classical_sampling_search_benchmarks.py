...
Outputs (folders):
  ...
  Carbon footprints/<outdir>/   (carbon from Performance runtimes only)

CLI mirrors your previous scripts.
"""
...
def make_marked_set(N: int, M: int, rng: np.random.Generator):
    """Return boolean mask of length N with exactly M marked items."""
    if M < 0 or M > N:
...
def grover_like_budget_k(N: int, M: int) -> int:
    """Equal budget: k ≈ round((π/4)*sqrt(N/M)); at least 1 if M>0."""
    if M <= 0:
        return 0
