...
Accuracy metric:
  For each random initial state |ψ>, evolve with user Trotter steps (coarse)
...
Performance & Carbon:
  Runtime/peak memory are measured on the **coarse** simulation only.
  Carbon is computed from Performance runtime only (as in your rules).

Outputs:
  Folders created in CWD:
    Performance/  Scalability/  Reliability/  Carbon footprints/<outdir>/
  Each benchmark folder contains an Excel workbook and 2 PNG plots.
