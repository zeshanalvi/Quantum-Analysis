...
CLI matches your previous style. Enable adaptation with:  --adapt-stepsize
"""
...
def _autocov_fft(x: np.ndarray) -> np.ndarray:
    """Fast autocovariance using FFT for 1D zero-mean series x."""
    n = len(x)
    if n <= 1:
...
def dual_averaging_warmup(q_init, warmup, L, eps_init, target_accept,
    ...
    """
    Dual-averaging (Hoffman & Gelman, 2014) to tune ε during warmup.
    Returns (q_final, eps_final, accept_mean, grad_evals_used).
    """
    if warmup <= 0:
...
def run_single(d, steps, warmup, L, eps_input,
    ...
    # --- Warmup (with optional ε adaptation) ---
    eps = float(eps_input)
