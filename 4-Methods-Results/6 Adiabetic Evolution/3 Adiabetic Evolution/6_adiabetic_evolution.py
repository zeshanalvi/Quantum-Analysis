...
# -*- coding: utf-8 -*-
"""
GPU-accelerated Empirical Evaluation: Adiabatic Quantum Simulation (Noiseless) – Quantum Baseline
Level: Master's Research Script

This script implements Adiabatic Quantum Simulation for 1D Transverse-Field Ising Model
using ideal statevector simulation and produces the same deliverables:
  1) Performance     – runtime, memory, simulation accuracy
...
def Uzz(theta: float) -> np.ndarray:
    """ZZ rotation gate: exp(-i * theta * (Z ⊗ Z))"""
...
def _run_single_experiment(n_qubits: int, steps: int, seed: int) -> dict:
    ...
    try:
        ...
        print(f"❌ Experiment failed for n_qubits={n_qubits}, steps={steps}, seed={seed}: {e}")
        error = float('inf')
        success = False
...
def run_adiabatic_simulation_experiment():
    ...
    print(f"🚀 Starting Adiabatic Quantum Simulation experiments on {get_device_name()}")
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_job):
                try:
                    ...
                    print(f"❌ Job failed n_qubits={job[0]}, steps={job[1]}, seed={job[2]}: {e}")
    else:
    ...
    if not all_rows:
        ...
        for job in jobs:
            try:
                ...
                print(f"❌ Sequential fallback also failed for n_qubits={job[0]}, steps={job[1]}, seed={job[2]}: {e}")
    ...
    print("\n" + "="*60)
    print("🎉 ADIABATIC QUANTUM SIMULATION EXPERIMENT COMPLETE - RESULTS SUMMARY")
    print("="*60)
    ...
    print(f"🔢 Qubits tested:   {sorted(perf_df['n_qubits'].unique())}")
    print(f"🔄 Steps tested:    {sorted(perf_df['steps'].unique())}")
    print(f"🎯 Success rate:    {success_rate * 100:.1f}%")

    if success_count > 0:
        success_df = perf_df[perf_df["success"]]
        print(f"📏 Mean error:      {success_df['error'].mean():.2e}")
        print(f"⏱️  Mean runtime:    {success_df['runtime_s'].mean():.3f}s")
        print(f"🧮 Max dimension:   {success_df['dimension'].max()}")
    else:
        print(f"📏 Mean error:      N/A (no successful runs)")
        print(f"⏱️  Mean runtime:    N/A (no successful runs)")
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Adiabatic Quantum Simulation Benchmark on Kaggle")
    print("=" * 50)
    ...
    if results_df is not None:
        ...
        print("\n✅ All Adiabatic Quantum Simulation deliverables generated successfully!")
        ...
        print("❌ Adiabatic Quantum Simulation benchmark failed - no results generated")
