# quantum_hamiltonian_sim_noiseless.py
# quantum_trotterization_noiseless.py
# every Trotterization run is a Hamiltonian simulation, 
# but not every Hamiltonian simulation uses Trotterization 
# (alternatives include Taylor/LCU, qubitization/QSP, Chebyshev/Krylov, variational simulation, etc.).
# Ideal (noiseless) quantum statevector simulator for first-order Trotterization.
# Metrics & outputs mirror the classical script:
#   - Performance, Scalability, Reliability, Carbon
#   - One Excel + 2–3 PNGs per metric, each in its own folder
# Carbon is computed STRICTLY from Performance runtimes.
...
# ---------------------------
# Gate construction (ideal simulator)
# ---------------------------
...
def apply_2q_nn(U: np.ndarray, psi: np.ndarray, n: int, i: int) -> np.ndarray:
    ...
    psi = psi.reshape([2] * n)
    psi = np.moveaxis(psi, (i, i + 1), (0, 1))        # (2,2,2,...)
    psi = (U @ psi.reshape(4, -1)).reshape([2, 2] + [2] * (n - 2))
    psi = np.moveaxis(psi, (0, 1), (i, i + 1))
    return psi.reshape(-1)

# ---------------------------
# Hamiltonian for exact solution (for reliability only)
# ---------------------------
...
def build_full_hamiltonian(n: int, J: float, h: float) -> np.ndarray:
    ...
    for i in range(n - 1):
        H += J * (two_qubit_op(i, i + 1, X, X) + two_qubit_op(i, i + 1, Y, Y) + two_qubit_op(i, i + 1, Z, Z))
    for i in range(n):
        H += h * single_qubit_op(i, Z)
    return H

# ---------------------------
# Simulation kernels (ideal)
# ---------------------------
...
# ---------------------------
# Plot helpers (same as classical)
# ---------------------------
...
def plot_runtime_vs_steps(perf_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Runtime vs Trotter steps (ideal simulator)")
    ax.legend()
...
def plot_memory_vs_steps(perf_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Memory vs Trotter steps (ideal simulator)")
    ax.legend()
    save_fig(fig, out_path)
...
def plot_error_vs_steps(rel_df: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("Reliability: Error vs steps (ideal simulator)")
    ax.legend()
    save_fig(fig, out_path)
...
def plot_error_heatmap(raw_err: pd.DataFrame, out_path: str) -> None:
    ...
    ax.set_title("log10(Error): n vs m (ideal simulator)")
...
# ---------------------------
# Carbon logic (same as classical, with 'latest' year selection)
# ---------------------------
...
def main():
    parser = argparse.ArgumentParser(description="Ideal quantum Trotter simulator (noiseless) with four metrics and per-metric outputs.")
    # Model/grid
    ...
    # Carbon / file inputs (same CLI as classical)
    ...
    # CO2 file resolution (Desktop default)
    desktop = os.path.join(os.environ.get("USERPROFILE", ""), "Desktop")
    defaults = [
    ...
    print("[INFO] Running ideal simulator benchmarks ...")
    t_all0 = time.perf_counter()
    perf_df, err_raw = run_benchmarks(
        n_list=n_list, m_list=m_list, repeats=args.repeats,
        J=args.J, h=args.h, T=args.T, rng_seed=args.seed,
    ...
    print(f"[DONE] Ideal simulator completed in {time.perf_counter() - t_all0:.2f}s")
