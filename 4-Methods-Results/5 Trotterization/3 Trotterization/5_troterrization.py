...
def expm_hermitian(H: cp.ndarray, dt: float) -> cp.ndarray:
    """Matrix exponential for Hermitian matrices using eigenvalue decomposition"""
    if HAS_GPU:
        # CuPy eigh
    ...
    try:
        ...
        if n_qubits <= 8:  # Limit exact computation to avoid memory issues
            try:
                ...
                print(f"⚠️  Exact computation failed for n={n_qubits}: {e}")
                error = -1.0  # Mark as failed

        success = error >= 0 and (error < 0.1 if n_qubits <= 8 else True)

    except Exception as e:
        print(f"❌ Trotter simulation failed for n={n_qubits}, steps={trotter_steps}: {e}")
        error = -1.0
        success = False
...
def run_quantum_hamiltonian_experiment():
    ...
    def _consume(row):
        ...
        print(f"  ✅ n={row['n_qubits']}, steps={row['trotter_steps']}, seed={row['seed']} "
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_job):
                try:
                    ...
                    print(f"❌ Job failed n={job[0]}, steps={job[1]}, seed={job[2]}: {e}")
    else:
    ...
    print(f"🔬 Qubit range:     {perf_df['n_qubits'].min()}-{perf_df['n_qubits'].max()}")
    print(f"🔬 Trotter steps:   {sorted(perf_df['trotter_steps'].unique())}")
