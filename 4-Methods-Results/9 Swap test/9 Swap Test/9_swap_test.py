...
# -*- coding: utf-8 -*-
"""
GPU-accelerated Empirical Evaluation: Quantum SWAP Test (Noiseless) – Quantum Baseline
...
def _run_single_experiment(dimension: int, pairs: int, seed: int) -> dict:
    ...
    try:
        ...
        print(f"❌ Experiment failed for dimension={dimension}, pairs={pairs}, seed={seed}: {e}")
        metrics = {
            "mse": float('inf'),
...
def run_swap_test_experiment():
    ...
    print(f"🚀 Starting Quantum SWAP Test experiments on {get_device_name()}")
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_job):
                try:
                    ...
                    print(f"❌ Job failed dimension={job[0]}, pairs={job[1]}, seed={job[2]}: {e}")
    else:
    ...
    if not all_rows:
        ...
        for job in jobs:
            try:
                ...
                print(f"❌ Sequential fallback also failed for dimension={job[0]}, pairs={job[1]}, seed={job[2]}: {e}")
    ...
    print("\n" + "="*60)
    print("🎉 QUANTUM SWAP TEST EXPERIMENT COMPLETE - RESULTS SUMMARY")
    print("="*60)
    ...
    print(f"🔬 Dimensions tested: {sorted(perf_df['dimension'].unique())}")
    ...
    if success_count > 0:
        ...
        print(f"📊 Mean cosine²:   {success_df['mean_cosine_sq'].mean():.3f}")
    else:
        print(f"📏 Mean MSE:        N/A (no successful runs)")
        print(f"📐 Mean fidelity:   N/A (no successful runs)")
        print(f"📊 Mean cosine²:   N/A (no successful runs)")
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Quantum SWAP Test Benchmark on Kaggle")
    print("=" * 50)
    ...
    if results_df is not None:
        ...
        print("\n✅ All Quantum SWAP Test deliverables generated successfully!")
        ...
        print("❌ Quantum SWAP Test benchmark failed - no results generated")
